...
1. `ghttp`
    ...
    - 新增`Request.GetCtx/GetCtxVar/SetCtxVar/Context`上下文变量管理方法，用于请求内部的上下文变量特性：https://
    - 新增`Request.GetUploadFile/GetUploadFiles`方法，以及`UploadFile`类型，用于简化文件上传：https://
    - 新增`Request.GetPage`方法，用于便捷地获得分页对象：https://
    - 改进`Response.Redirect*`方法，增加自定义的跳转HTTP状态码参数。
    - 改进`CORS`特性，完善跨域功能处理，并完全遵守`W3C`关于`OPTIONS`请求方法的规范约定：https://
...
## `database`
1. `gdb`
    - 代码重构改进，增加`Driver`驱动接口设计，方便开发者自定义驱动实现。新增`Register`包方法，用于开发者注册自定义的数据库类型驱动：https://
    - 新增`GetArray`接口及实现，用于获取指定字段列的数据，构造成数组返回：https://
    - 新增`InsertIgnore`接口及实现，用于写入时忽略写入冲突，仅对`mysql`数据库类型有效：https://
    - 新增`Schema`接口及实现，用于动态切换并获取指定名称的数据库对象：https://
    - 新增`FieldsStr/FieldsExStr`模型方法，用于获取表字段，并构造成字符串返回：https://
    - 新增`LockUpdate/LockShared`模型链式操作方法，用于实现悲观锁操作：https://
    - 改进`Where/Data`方法对更新参数输入方式的支持，提高灵活性：https://
    - 查询结果对象`Result`新增`Array`方法，用于获得指定字段的数值，构造成数组返回：https://
...
## `os`
1. `glog`
    - 增加`Rotation`日志滚动切分特性，新增按照文件大小进行日志切分，并支持切分文件数量限制（清理）、对日志文件进行自动压缩、可自定义压缩级别（`1-9`）、支持对切分文件过期时间清理：https://
    - 新增`LevelPrefixes`特性，支持对日志级别的前缀名称进行自定义：https://
